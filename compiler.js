...
var compiler = {
    ...
    help: function () {
        ...
        this.log([
            ...
            '    --namespace',
            '    [grey]using namespace not seajs/require[/grey]',
    ...
    engine: (function () {
        ...
        var toModule = function (code, helpersPath, that) {
            //TODO STEP3
            var pkg = that.options['namespace'],
                useNamespace = !!pkg,
                namespace = pkg + '.' + (function(file){
            ...
            var require = function(file){
                if(useNamespace){
                    ...
                    return  'require(\'' + fixPath(file) + '\')';
                }
            };

            //TODO STEP5  dependencys
            var dependencies = [];
            parseDependencies(render).forEach(function (file) {
                dependencies.push(
                    '\'' + fixPath(file) + '\': '  + require(file)
                );
            });
            ...
            //TODO STEP6 helpers
            if (helpersPath) {
                helpers =  require( pkg + '.' + that.TEMPLATE_NAME);
            } else {
                helpers = [];
        ...
        return function (source, helpersPath, that) {
            return toModule(source, helpersPath, that);
        }
    ...
    writeHelpers: function () {
        ...
        if(this.options['namespace']){
            module = ';(function () { '
                + 'namespace(\'' + this.options['namespace'] + '.' +  this.HELPERNAMESAPCE + '\','
                + helpers
                + ')})();'
        }else{
    ...
    compile: function (file) {
        ...
        try {
            ...
            var code = this.engine(source, name, {
                ...
                file : info_output,
                TEMPLATE_NAME : this.HELPERNAMESAPCE
    ...
    init: function () {
        ...
        while (args.length > 0) {
            ...
            switch (v) {
                ...
                ////TODO STEP1!!!
                case '--namespace':
                    options.namespace = args.shift().toLowerCase();
//                    options.cloneHelpers = true;
                    break;
